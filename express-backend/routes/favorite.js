...
// This route is used to get user's favorite recipes
...
module.exports = (db) => {
  router.post("/", (req, res) => {
    ...
    return db
      .query(
        `SELECT DISTINCT users.id, recipes.id, recipes.name, recipes.description, recipes.img_url
    FROM favourites
    FULL JOIN users ON users.id = user_id
    FULL JOIN recipes ON recipes.id = recipe_id
    WHERE users.email = $1 AND is_favourite = true;`,
        [email]
      )
      .then((data) => {
  ...
  router.post("/:id/favourites", (req, res) => {
    ...
    WHERE email = $1;`,
      ...
      WHERE user_id = $1 AND recipe_id = $2;`,
          ...
          VALUES ($1, $2, $3)
          RETURNING *;`,
                [user_id, recipe_id, is_favourite]
              )
                .then((data) => {
          ...
          RETURNING *;`,
                [is_favourite, user_id, recipe_id]
              )
                .then((data) => {
                  const favourited = data.rows;
