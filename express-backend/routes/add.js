...
// This route is needed for adding a recipe by the user
        ...
        `,
          ...
          .then((data) => {
            ...
            for (let index in ingredients) {
              ...
              db.query(`SELECT id FROM ingredients WHERE name = $1;`, [
                eachIngredient,
              ])
                  ...
                  if (ingredientIdData.rows.length === 0) {
                    db.query(
                      ...
                      .then((ingredientId) => {
                        if (unit.length > 0) {
                          db.query(
                            `INSERT INTO measurements (ingredient_id,recipe_id,amount,unit) VALUES($1,$2,$3,$4) RETURNING *;`,
                            [
                          ...
                          db.query(
                            `INSERT INTO measurements (ingredient_id,recipe_id,amount) VALUES($1,$2,$3) RETURNING *;`,
                            [ingredientId.rows[0].id, recipe_id, amount[index]]
                          )
                            .then((measurement) => {
                              const recipes = { name, description, img_url };
                              //  res.json({ recipes });
                    ...
                    if (unit.length > 0) {
                      ...
                      db.query(
                        `INSERT INTO measurements (ingredient_id,recipe_id,amount) VALUES($1,$2,$3) RETURNING *;`,
                        [ingredientIdData.rows[0].id, recipe_id, amount[index]]
                      )
                        .then((measurement) => {
                          const recipes = { name, description, img_url };
                          //  res.json({ recipes });
